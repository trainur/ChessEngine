...
public class Extensions_9 : ChessAgent
{
    ...
    protected override SearchResult ChooseMove(BoardState state)
    {
        ...
        if (transpositionTable.Count > 500_000) transpositionTable.Clear(); 
    ...
    private int Minimax(BoardState state, int depth, int alpha, int beta)
    {
        ...
        foreach (Move move in moves)
        {
            ...
            int ext = GetExtension(state, move, depth);

            int score = Minimax(state, depth - 1 + ext, alpha, beta);
            positionHistory[state.ZobristKey]--;
    ...
    private int Quiescence(BoardState state, int alpha, int beta)
    {
        ...
        // Stand pat is not allowed when in check, threat needs to be resolved
        if (state.IsCheck())
        {
    ...
    int KingPawnShield(ulong king, ulong pawns, bool isWhite)
    {
        ...
        bool looksCastled = file <= 2 || file >= 6;
        if (!looksCastled) return 0;
    ...
    private static int GetPSTBonus(short[] table, ulong pieces, bool isWhite)
    {
        ...
        while (bb != 0)
        {
            ...
            // Online PST table reflects the PST for black.
            int index = isWhite ? (sq + 56) - (sq / 8) * 16 : sq;
            bonus += table[index];
        }
    ...
    private int GetExtension(BoardState state, Move move, int currentDepth)
    {
