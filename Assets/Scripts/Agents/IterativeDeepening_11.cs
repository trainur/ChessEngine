...
public class IterativeDeepening_11 : ChessAgent
{
    ...
    private struct Line
    {
        ...
        public void Set(Move firstMove, Line childLine)
        {
            ...
            for (int i = 0; i < childLine.CMove; i++)
    ...
    private int Negamax(ref BoardState state, int depth, int alpha, int beta, ref Line pv, Line previousPv, int ply)
    {
        // Draw state
        ...
        foreach (Move move in moves)
        {
            ...
            int score = -Negamax(ref state, depth - 1 + ext, -beta, -alpha, ref childPv, previousPv, ply + 1);
