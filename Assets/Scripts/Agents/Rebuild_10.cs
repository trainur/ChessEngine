...
// Attempt to fix Extensions_9
// Minimax swapped to Negamax
// Position histories are baked into Agent base class past this point of development.
// Includes tactical draws.

public class Rebuild_10 : ChessAgent
{
    ...
    protected override SearchResult ChooseMove(BoardState state)
    {
        ...
        {
            UndoInfo undoInfo = MakeSearchMove(ref state, move);

            int score = -Negamax(ref state, searchDepth - 1, -beta, -alpha);

            UnmakeSearchMove(ref state, move, undoInfo);
    ...
    private int Negamax(ref BoardState state, int depth, int alpha, int beta)
    {
        // Draw state
        ...
        if (depth == 0) return Quiescence(ref state, alpha, beta);
        ...
        foreach (Move move in moves)
        {
            ...
            int ext = GetExtension(ref state, move, depth);

            int score = -Negamax(ref state, depth - 1 + ext, -beta, -alpha);
    ...
    {
        static int GetMoveOrderScore(Move move)
        {
            int score = 0;
    ...
    private int GetExtension(ref BoardState state, Move move, int currentDepth)
    {
